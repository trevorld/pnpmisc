...
draw_wallet_cover <- function(b = 0, front = NA, back = NA) {
	...
	gp_arrow <- arrow(angle = 30, length = unit(0.125, "in"), ends = "last", type = "closed")
...
draw_wallet_spine <- function(b = unit(0, "in"), spine = NULL) {
	...
	grid.circle(
		x = unit(rep(c(0, 2, 6, 8), each = 2L), "in"),
	...
	grid.text(
		"Folds 1",
		x = unit(c(1, 1, 7, 7), "in"),
		y = unit(c(0.25, 9.75, 0.25, 9.75), "in"),
		gp = gp_text
	)
	...
	grid.text("Folds 2", x = unit(c(4.5, 3.5), "in"), y = unit(c(1.75, 7.75), "in"), gp = gp_text)

	mask_spine <- as.mask(polygonGrob(
		x = unit(
			c(0, 0, 0 + 2 * b, 2 + 2 * b, 6, 8, 8 + 2 * b, 8 + 2 * b, 8, 6, 2 + 2 * b, 0 + 2 * b),
			"in"
		),
		y = unit(
			c(6 + 2 * b, 0, 0, 2, 2, 0, 0, 6 + 2 * b, 6 + 2 * b, 4 + 2 * b, 4 + 2 * b, 6 + 2 * b),
			"in"
		),
		gp = gpar(col = NA, fill = "white")
	))
