...
### `bm_split_layout()` and `grid_add_layout()`
...
* You can use functions like `bm_split_layout()` and `grid_add_layout()` to create a pdf of the booklet pages all oriented up.
...
# <https://buttonshygames.itch.io/a-nice-cuppa>
input <- "A Nice Cuppa - PNP.pdf"
output <- "a_nice_cuppa_rules.pdf"
layout <- layout_preset("button_shy_rules")
bml <- input |> pdf_render_bm_pixmap() |>
    bm_split_layout(layout = layout)

layout <- transform(layout, angle = 0, name = paste0("page_", 1:8))
...
library("pnpmisc")
# Download "Numbsters"
# <https://buttonshygames.itch.io/numbsters>
...
``` r
library("pnpmisc")
# Download Birdscaping from <https://boardgamegeek.com/boardgame/366484/birdscaping/files>
input <- list.files(pattern = "^Birdscaping_1.7")
output <- "birdscaping_cards.pdf"
pdf_combine(input) |>
	pdf_pad_paper(output)
```
...
pdf_rm_crosshairs(input, output, pages = "odd", layout = "poker_3x2_bleed")
```
